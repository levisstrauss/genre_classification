...
def test_kolmogorov_smirnov(data, ks_alpha):
    ...
    for col in columns:
        sample1[col] = pd.to_numeric(sample1[col], errors='coerce')
        sample2[col] = pd.to_numeric(sample2[col], errors='coerce')

        # Remove NaN values
        sample1 = sample1.dropna(subset=[col])
        sample2 = sample2.dropna(subset=[col])
    ...
    for col in columns:
        ...
        assert not np.isnan(p_value), f"p-value is NaN for column {col}"
        assert p_value > alpha_prime, f"Column {col} failed K-S test with p-value {p_value}"